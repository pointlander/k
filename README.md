...
Group words are a 2-bit tagged prefix-free code. Higgs/breaking is in the same grammar: the SM pays one fund (the doublet); \(SU(5)\) pays adj+fund (\(\mathbf{24}+\mathbf{5}\)); \(SO(10)\) pays adj+fund+spinor. Conditioned on an SM-like sample the minimizer is still \(SU(5)\) (length 29 vs SM 38 vs \(SO(10)\) 36). That is a public failure of “the SM is the unique shortest plugin” in this encoding, not a 2-bit discount. Raw \(N\)-lists still lose.
