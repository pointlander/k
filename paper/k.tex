...
\paragraph{9. Shortest matter plugin is a GUT, not the SM product.}
In the plugin language of the analog, $SU(5)$ plus $\mathbf{24}+\mathbf{5}$ is shorter than $SU(3)\times SU(2)\times U(1)$ even after Higgs is in-word.
A world whose shortest plugin compatible with the low-energy sample was the SM product, with no shorter GUT generator, would falsify this encoding.
...
A plugin is gravity (the dimensionality word) plus $G$, a fermion-rep count, $n_{\mathrm{gen}}$, $\deg V$, and a Higgs word $\mathtt{unary}(k)\,\mathrm{rep}^{k}$ with $\mathrm{rep}\in\{\mathrm{fund},\mathrm{afund},\mathrm{adj},\mathrm{spinor}\}$.
The SM pays one fund (the doublet).
$SU(5)$ pays adj$+$fund (the $\mathbf{24}$ and the $\mathbf{5}$).
$SO(10)$ pays adj$+$fund$+$spinor (the $\mathbf{45}$, $\mathbf{10}$, and $\mathbf{16}$).
Adjoint is a short program given $G$ (``all generators''), not unary-$24$.
A raw landscape of $N$ vacua is $N$ concatenated copies, length $\Theta(N)$; a generator that prints $N$ copies costs $O(\log N)$.
Conditioned on a low-energy sample that requires $4$d Einstein and SM quantum numbers---an input, like Choquet--Bruhat---the lengths are $29$ ($SU(5)$), $36$ ($SO(10)$), $38$ (SM product), $312$ (raw $16\times$SM).
The minimizer is $SU(5)$, merge mass $0.99$.
Charging the Higgs sector in the grammar did not make the SM the unique shortest plugin: the product group is still nine bits longer than $SU(5)$ plus a $24{+}5$.
That is a public failure of ``the SM is the minimizer'' in this encoding, not a 2-bit discount.
Raw $N$-lists still lose.
The algebraic core---Higgs lengths $4$, $7$, $10$; theory lengths $38$, $29$, $36$; $SU(5)$ dominates the SM product by $2^{9}$; Kraft on the named encodings---is a Lean 4 module in the repository.
...
\begin{enumerate}
  ...
  A finite plugin language with Higgs in-word is in the repository.
  After charging $SU(5)$'s $\mathbf{24}{+}\mathbf{5}$ and $SO(10)$'s $\mathbf{45}{+}\mathbf{10}{+}\mathbf{16}$, the minimizer is still $SU(5)$, not the SM product.
  The remaining task is a finer grammar (Dynkin labels, doublet-triplet operators) that either selects the SM or is abandoned.
