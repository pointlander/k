...
The GR-machine is a Turing machine with an Einstein primitive.
If that primitive is itself Turing-computable (numerical GR to finite resolution, which is all sampling ever needs), $\UTM$ is an ordinary UTM and $\KG$ differs from $K$ by $O(1)$.
The continuum is never computed pointwise.
It is named by a program that \emph{refers} to an Einstein development.
...
The algebraic core---Friedmann constraint, de~Sitter as a fixed point of the minisuperspace vector field, Einstein as a bit-saving compressor, and $2^{\beta}$ dominance of on-shell vacuum over off-shell data---is a Lean 4 module in the repository.
...
\begin{enumerate}
  ...
  Decide the no-boundary versus tunneling contest as a $\KG$ comparison.

  \item \textbf{Standard Model as plugin.}
  State a precise minimization problem whose solution would \emph{be} the Standard Model: a shortest matter Lagrangian, in a fixed encoding of field theories, compatible with a given low-energy sample.

  \item \textbf{Evaporation.}
  Compute, in a controlled analog or a minisuperspace model, whether $\KG$ of an evaporating hole plus radiation remains equal to $A_{\mathrm{initial}}/4\ell_{P}^{2}$ along the history.
  This is the information problem as a problem in gravitational Kolmogorov complexity.

  \item \textbf{Complexity corrections.}
  Bound $\KG(a)-\KG(b)$ in a concrete few-qubit interferometer and say whether~\eqref{eq:born-corr} is experimentally accessible.

  \item \textbf{Dimensionality.}
  Show that $3+1$ is the shortest well-posed Einstein compressor, in the sense of Dzhunushaliev's complexity-driven dimensional reduction~\cite{dzhunushaliev1998,dzhunushaliev2002} made into a theorem about $\KG$.
\end{enumerate}
